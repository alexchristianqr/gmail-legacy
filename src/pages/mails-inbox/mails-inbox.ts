...
export class MailsInboxPage implements OnDestroy {
  ...
  fnViewSearch () {
    (this.modalCtrl.create(SearchPage,
        {database: SHARED_PREFERENCE.DB.DI})).present()
  }

  fnViewCreate () {
    (this.modalCtrl.create(CreatePage,
        {database: SHARED_PREFERENCE.DB.DI})).present()
  }

  fnFetch () {
    this.dialogService.showLoading()
    this.storage.get('DATABASE_INBOX').then((data) => {
  ...
  fnClean () {
    this.dialogService.dialogQuestion('Warning',
        'Do you want to clean the database INBOX?', () => {
