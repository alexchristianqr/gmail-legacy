...
export class SearchPage {
    ...
    getItems(e: any) {
        ...
        this.items = [];
        this.storage.get(this.navParams.data.database).then((data) => {
            if (data != null) {
                if (text.toLowerCase().length == 0) {
                    return this.items = [];
                } else {
                    return this.items = data.filter((objeto) => {
                        if (objeto.name.toLowerCase().indexOf(text.toLowerCase()) > -1 || objeto.subject.toLowerCase().indexOf(text.toLowerCase()) > -1 || objeto.message.toLowerCase().indexOf(text.toLowerCase()) > -1) {
                            return true;
                        }
                    });
                }
