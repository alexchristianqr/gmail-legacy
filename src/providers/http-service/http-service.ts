...
export class HttpServiceProvider {
  ...
  loadPreferences (self: any) {
    self.storage.get('SHARED_PREFERENCE').then((data) => {
      if (data != null) {
  ...
  create (self: any) {
    // Get storage
    let msg = ''
    self.storage.get(self.params.database).then((data) => {
      data = (data == null) ? [] : data
      data.unshift(self.params)
      // Set storage
      self.storage.set(self.params.database, data).then((data) => {
  ...
  remove (self: any, database: any): void {
    self.storage.get(database).then((data) => {
      data.forEach((item, i) => {
        if (self.index === i) {
          data.splice(i, 1)
